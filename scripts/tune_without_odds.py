"""
Hyperparameter tuning WITHOUT odds as features (baseline approach).
...
def evaluate_config(params, df, train_idx, val_idx, ev_threshold=0.02):
    ...
    try:
        ...
        print(f"  Features: {len(feature_cols)} (WITHOUT odds)")
...
def grid_search():
    ...
    print("="*80)
    print("HYPERPARAMETER TUNING: WITHOUT ODDS AS FEATURES")
    print("="*80)
    ...
    print(f"\nTesting {total_configs} configurations WITHOUT odds features...")
    ...
    if not successful_results:
        ...
        # Save all results
        results_path = 'models/metadata/hyperparameter_tuning_WITHOUT_odds.json'
        os.makedirs(os.path.dirname(results_path), exist_ok=True)

        with open(results_path, 'w') as f:
            json.dump({
                'timestamp': datetime.now().isoformat(),
                'approach': 'WITHOUT odds as features',
    ...
    print("\n" + "="*80)
    print("BEST CONFIGURATION FOUND (WITHOUT ODDS)")
    print("="*80)
    ...
    # Prepare data - same exclusions as training
    feature_cols = [col for col in df.columns if col not in [
        'game_id', 'goalie_id', 'game_date', 'over_hit',
    ...
    if best_test_threshold and best_test_roi > 0:
        print("\n" + "="*80)
        print("BEST TEST RESULT (WITHOUT ODDS)")
        print("="*80)
